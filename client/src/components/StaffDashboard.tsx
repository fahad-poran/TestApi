...
const StaffDashboard = () => {
  ...
  const handleSubmitInvoice = async () => {
    ...
    try {
      ...
      // Refresh product list (stock has changed)
      try {
        const data = await getProducts(token!);
        setProducts(data);
      } catch {}

      // Navigate to invoice history so the user can see the new invoice
      const role = localStorage.getItem('role');
      navigate(role === 'Admin' ? '/admin/invoices' : '/staff/invoices');
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">Select products and create a new invoice</p>
      </div>
